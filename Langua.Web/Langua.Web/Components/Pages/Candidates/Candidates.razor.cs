...
using Langua.Web.Components.Pages;
...
namespace Langua.Web.Components.Pages.Candidates
{
    ...
    public partial class CandidatesComponent : BasePage
    {
        ...
        {
            var result = await dialogService.OpenAsync<Langua.Web.Components.Pages.Candidates.EditCandidate>("Edit Componenet", new Dictionary<string, object> { { "Id", candidat.Id } });
            await grid0.Reload();
        }
        public async Task Add()
        {
           var result = await dialogService.OpenAsync<Components.Pages.Candidates.AddCandidate>("Add new candidate", null, new DialogOptions { Width = "50vw",  ShowClose = true });
            await grid0.Reload();
        }
    }
